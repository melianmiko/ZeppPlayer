...
Based on https://raw.githubusercontent.com/magicien/TGAImage/master/src/TGAImage.js
Modified to support ZeppOS specific RRRRRGGG GGGBBBBB colormap
and migrate to NodeJS canvas.
...
export class TGAImage {
    ...
    _getRGBA(buf, offset, depth) {
        if (depth === 15) {
            ...
            const r = Math.floor(255/31 * ((pixel & 0xF800) >> 11));
            const g = Math.floor(255/63 * ((pixel & 0x7E0) >> 5));
            const b = Math.floor(255/31 * (pixel & 0x1F));
            const a = 255;
            return [r, g, b, a]
        } else if (depth === 24) {
